...
function calcRefund(purchase,sub,asOf=new Date()){
  const matchedItem=(purchase?.items||[]).find(i=>String(i.productName)===String(sub?.productName)&&String(i.typeName)===String(sub?.typeName));
  const totalDays=Math.max(0,Number(sub?.durationDays||matchedItem?.durationDays||0));
