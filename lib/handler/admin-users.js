...
function repairSubscriptionDurations(user) {
  ...
  for (const sub of Array.isArray(user.subscriptions) ? user.subscriptions : []) {
    if (!sub || sub.durationDays === null || sub.durationDays === undefined) continue;
    const purchase = purchases.find(p => p.id === sub.purchaseId);
    const item = purchase?.items?.find(i => String(i.productName) === String(sub.productName) && String(i.typeName) === String(sub.typeName));
    const original = Number(item?.durationDays);
    if (Number.isFinite(original) && original > 0 && Number(sub.durationDays) < original) sub.durationDays = original;
  }
}
...
function ensureUserCollections(user) {
  ...
  repairSubscriptionDurations(user);
...
function adjustSubscription(subscription, deltaMinutes, setRemainingMinutes) {
  ...
  // durationDays is the original purchased duration and must never be replaced by remaining time.
